...
#define TAG "ti.notifications"

using namespace v8;

static Persistent<Object> bindingCache;

static Handle<Value> TiNotifications_getBinding(const Arguments& args)
{
	HandleScope scope;

	if (args.Length() == 0) {
		return ThrowException(Exception::Error(String::New("TiNotifications.getBinding requires 1 argument: binding")));
	}
	...
	titanium::bindings::BindEntry *extBinding = ::TiNotificationsBindings::lookupGeneratedInit(
...
static void TiNotifications_init(Handle<Object> exports)
{
	HandleScope scope;
	...
	exports->Set(String::New("getBinding"), FunctionTemplate::New(TiNotifications_getBinding)->GetFunction());
}

static void TiNotifications_dispose()
{
	HandleScope scope;
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			::TiNotificationsBindings::lookupGeneratedInit(*binding, bindingLength);
...
static titanium::bindings::BindEntry TiNotificationsBinding = {
	"ti.notifications",
	TiNotifications_init,
	TiNotifications_dispose
};

// Main module entry point
extern "C" JNIEXPORT void JNICALL
Java_ti_notifications_TiNotificationsBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("ti.notifications", &TiNotificationsBinding);
	titanium::KrollBindings::addExternalLookup(&(::TiNotificationsBindings::lookupGeneratedInit));
}
